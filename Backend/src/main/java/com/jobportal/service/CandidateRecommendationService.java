...
public class CandidateRecommendationService {
    ...
    public List<AICandidateRankingEntity> getRecommendedJobsForCandidate(String candidateUid) throws Exception {
        ...
        // In a real prod environment we would use a vector database or keyword filtering first to narrow down from 10k jobs to top 50, then Gemini them.
        List<JobEntity> activeJobs = jobRepository.findAll(); // Assuming this returns a limited active set for this prototype
        ...
        for (JobEntity job : activeJobs) {
            if ("ACTIVE".equals(job.getStatus())) {
                try {
                    // Lazy evaluate
                    AICandidateRankingEntity ranking = rankingService.getOrGenerateRanking(job.getJobId(), candidateUid);
